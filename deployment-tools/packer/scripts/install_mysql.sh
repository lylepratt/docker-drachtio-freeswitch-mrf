...
if [[ "$VARIANT" != "mini" ]]; then
  echo "skipping mysql installation for non-mini variant"
  exit 0
fi
...
if [[ "$DISTRO" == rhel* ]] ; then
  ...
  HOME=/home/ec2-user
  cd /tmp
  wget https://repo.mysql.com//mysql80-community-release-el${RHEL_RELEASE}.rpm
  sudo dnf install -y mysql80-community-release-el${RHEL_RELEASE}.rpm
